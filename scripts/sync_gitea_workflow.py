...
* `release.yml` deliberately has NO copy here, for the same reason in
  reverse: PyPI's trusted publishing only federates with GitHub, so a
  release job on a Gitea runner could not publish even if it ran.
...
#
# The same mechanism is why release.yml deliberately has no copy here:
# Gitea therefore never attempts it, which is what we want. PyPI's trusted
# publishing only federates with GitHub, so a release job on a Gitea runner
# could not publish even if it ran.
"""
