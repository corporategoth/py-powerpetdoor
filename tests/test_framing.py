...
def scan_counter(monkeypatch):
    ...
    is the direct measurement of the quadratic-rescan defect.
    """
    examined = [0]
...
class TestExtractFrames:
    ...
    def test_garbage_only_buffer_cleared(self):
        ...
        make the total depend on where the peer cut the packet.
        """
...
class TestFrameScanner:
    ...
    def test_dribbled_object_examines_each_character_once(self, scan_counter):
        ...
        This is the CPU-exhaustion vector: re-scanning the retained buffer
        on every ``data_received`` cost ~N^2/2 character steps for N
        delivered bytes. Each byte must be examined exactly once.
        """
        payload = '{"a": "' + "x" * 4000
        scanner = FrameScanner()
        for char in payload:
...
# ============================================================================
# Copy cost and chunk-invariance
# ============================================================================


class TestRetainedRemainderCost:
    """The retained remainder is never re-copied onto the incoming chunk."""
    ...
    def test_buffer_coalesces_exactly_two_pieces(self):
        ...
        correct. Every other ``.buffer`` assertion lands on 1 piece or on
        3+, so ``> 1`` -> ``> 2`` returns a **truncated** remainder
        (``'{"a": '`` for ``'{"a": "xy'``) with the rest of the suite green.
        """
        scanner = FrameScanner()
        scanner.feed('{"a": ')
        scanner.feed('"xy')
...
class TestDiscardedIsChunkInvariant:
    """``discarded`` counts non-whitespace only, so chunking cannot move it."""
